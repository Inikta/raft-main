...
from .utils import separate_address, unite_address
...
class PeerProtocol(DatagramProtocol):
    ...
    def connection_made(self, transport: DatagramTransport):
        ...
        if address:
            self.__connections[unite_address(address)] = transport

    def connection_lost(self, exception: Exception):
        if exception:
            self.__logger.exception(exception)
            
        self.__connections = {address: connection for address, connection in self.__connections.items() if connection.is_closing()}
    ...
    def send_to(self, message: dict[str, Any], address: str):
        ...
        transport.sendto(data, separate_address(address))
