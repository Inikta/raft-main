...
from .utils import separate_address
...
class Server:
    ...
    def __init__(self, address: str, network: list[str], server_name: str):
    ...
    def __create_protocols(self):
        ...
        basicConfig(format='%(levelname)s ' + self.__server_name + ' [%(name)s] %(message)s', level=NOTSET, stream=stdout)
    ...
    async def __start(self):
        ...
        (host, port) = separate_address(self.__address)
        ...
        for address in self.__network:
            await loop.create_datagram_endpoint(lambda: self.__peer_protocol, remote_addr=separate_address(address), family=AF_INET)
